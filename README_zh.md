...
dσ/dΩ_α    = Σ 事件 2π·b·P_tr(b,k)·δ(θ_α−θ_α(b,k))   α 角分布 (旁观者: α 在近点——位于束流前方轴线上——以近点切向速度 v_near=b·v_∞/D + 费米速度出发, 再经 ²³⁵Pa 库仑排斥传播到无穷远 → 渐近 θ_α, E_α; 近点 D(b)<R_int 落入核内的近正碰 b 事件被剔除——它们融合吸收, 不产生旁观 α; σ(E) 也应用同一 b_min 截断, 使三个截面归一化一致)
